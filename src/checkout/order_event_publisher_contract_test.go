...
func TestOrderEventPublisherContract(t *testing.T) {
	...
	messageHandlers := message.Handlers{
		"order-result message": func(states []models.ProviderState) (message.Body, message.Metadata, error) {
			...
			// Convert to the format that consumers expect (JSON)
			jsonObj, err := convertOrderResultToConsumerFormat(orderResult)
			if err != nil {
				return nil, nil, fmt.Errorf("failed to convert OrderResult to consumer format: %w", err)
			}
	...
	orderItems := []*pb.OrderItem{
		{
			...
			Cost: &pb.Money{
				CurrencyCode: "USD",
				Units:        15,
				Nanos:        990000000, // $15.99 per item
			},
		},
		{
			Item: &pb.CartItem{
