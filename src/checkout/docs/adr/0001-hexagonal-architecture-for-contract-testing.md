...
func TestOrderEventPublisherContract(t *testing.T) {
    // Clean, simple mock implementation
    mockPublisher := &MockOrderEventPublisher{}
    
    // Test business logic patterns through the port
    messageHandlers := message.Handlers{
        "order-result message": func(states []models.ProviderState) (message.Body, message.Metadata, error) {
            ...
            return orderToContractFormat(order), message.Metadata{"contentType": "application/json"}, nil
        },
    }
    
    // Verify contracts through the abstraction layer
